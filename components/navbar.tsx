							...
							<AnimatePresence mode="wait">
								{active && (
									<motion.div
										...
										exit={{ opacity: 0 }}>
										{links.map((link) => {
											const linkHref = (link.href === "register" || link.href === "login")
											...
											return (
												<Link
													key={link.id}
													href={linkHref}
													className="text-[16px] font-semibold capitalize text-[#260A2F] bg-secondary leading-tight tracking-tight hover:underline"
													onClick={() => setActive(!active)}>
													{t(link.href)}
												</Link>
											);
